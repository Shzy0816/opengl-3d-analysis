...
MyGLRenderContext *MyGLRenderContext::m_pContext2 = nullptr;
 ...
 * @param value1
 */
void MyGLRenderContext::SetParams(int paramType, int value0, int value1, std::string fileName) {
    LOGCATE("MyGLRenderContext::SetParamsInt paramType = %d, value0 = %d, value1 = %d , %s",
            paramType, value0, value1, fileName.c_str());
    ...
    if (paramType == SAMPLE_TYPE) {
        ...
        switch (value0) {
            ...
            case SAMPLE_TYPE_KEY_3D_MODEL:
                m_pCurSample = new Model3DSample(fileName);
                break;
            case SAMPLE_TYPE_KEY_3D_MODEL_ANIM:
                m_pCurSample = new Model3DAnimSample(fileName);
                break;
...
MyGLRenderContext *MyGLRenderContext::GetInstance(int i) {
    if (m_pContext == nullptr) {
        m_pContext = new MyGLRenderContext();
    }
    if (m_pContext2 == nullptr) {
        m_pContext2 = new MyGLRenderContext();
    }
    if (i == 0) {
        return m_pContext;
    } else {
...
void MyGLRenderContext::setViewMatrix(int eyeX, int eyeY, int eyeZ, int lookAtX, int lookAtY,
                                      int lookAtZ, int upX, int upY, int upZ) {
    m_pCurSample->setViewMatrix(eyeX, eyeY, eyeZ, lookAtX, lookAtY, lookAtZ, upX, upY,
                                upZ);
}

void MyGLRenderContext::AdjustPosition(int x, int y, int z) {
    m_pCurSample->AdjustModelPosition(x,y,z);
}
